...
<body class="bg-light" style="margin-top: 10vh;">
            ...
            <div class="row justify-content-center align-items-center my-5">
                ...
                <div class="col-md-10">
                    <div class="row">
                        ...
                        <!-- ฝั่งขวา: ญี่ปุ่น -->
                        <div class="col-md-6 text-center language-option" id="japan-option">
                            <div class="card h-100 p-4 language-card">
                                <div class="flag-container mx-auto mb-3">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const japanOption = document.getElementById('japan-option');

            // เลือกธงและการ์ดทั้งหมด
            const engFlag = engOption.querySelector('img');
            const japanFlag = japanOption.querySelector('img');
            const engCard = engOption.querySelector('.language-card');
            const japanCard = japanOption.querySelector('.language-card');

            // ดึงค่า path จาก URL ปัจจุบัน
            let path = window.location.pathname.split('/').filter(Boolean)[0];

            // หา ID จาก URL ถ้าเป็นไปได้ (สมมติว่า URL เป็น /contact/1 หรือ /survey/1)
            let contactId = 1; // ค่าเริ่มต้น, ควรเปลี่ยนตามความเหมาะสม
            const urlParts = window.location.pathname.split('/');
            if (urlParts.length > 2 && !isNaN(urlParts[2])) {
                contactId = urlParts[2];
            }

            // ถ้าไม่มี path ให้กำหนดค่าเริ่มต้นเป็น 'contact'
            if (!path) {
                path = 'contact';
            }

            console.log('Current path:', path); // เพิ่ม log เพื่อดีบัก

            // ตั้งค่าเริ่มต้นให้ภาษาอังกฤษถูกเลือก
            ...
            engFlag.addEventListener('click', function() {
                ...
                engCard.classList.add('selected');
                japanCard.classList.remove('selected');

                // เปลี่ยนภาษาเป็นอังกฤษตาม path
                if (path === 'contact') {
                    window.location.href = "{{ route('contact.show', ['contact' => ':contactId']) }}?lang=en".replace(':contactId', contactId);
                } else if (path === 'survey') {
                    window.location.href = "{{ route('survey.show', ['survey' => ':surveyId']) }}?lang=en".replace(':surveyId', contactId);
                } else {
                    window.location.href = "{{ route('changeLang', ['lang' => 'en']) }}";
                }
            });

            // เพิ่ม event listener สำหรับธงญี่ปุ่น
            japanFlag.addEventListener('click', function() {
                // ไฮไลท์การ์ดภาษาญี่ปุ่น
                japanCard.classList.add('selected');
                engCard.classList.remove('selected');

                // เปลี่ยนภาษาเป็นญี่ปุ่นตาม path
                if (path === 'contact') {
                    window.location.href = "{{ route('contact.show', ['contact' => ':contactId']) }}?lang=jp".replace(':contactId', contactId);
                } else if (path === 'survey') {
                    window.location.href = "{{ route('survey.show', ['survey' => ':surveyId']) }}?lang=jp".replace(':surveyId', contactId);
                } else {
            ...
            japanOption.addEventListener('click', function() {
                japanFlag.click();
            });
        });
